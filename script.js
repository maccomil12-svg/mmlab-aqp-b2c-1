...
// ==============================
// PROCESO: POPUPS DE LOS 5 PASOS
// ==============================
function closeProcessPopups() {
  $$('.process-popup').forEach((popup) => {
    popup.hidden = true;
  });
  $$('.process-step').forEach((step) => step.setAttribute('aria-expanded', 'false'));
}

$$('.process-step').forEach((step) => {
  step.addEventListener('click', (event) => {
    event.stopPropagation();
    const popup = document.getElementById(step.getAttribute('aria-controls'));
...
document.addEventListener('click', (event) => {
  if (!event.target.closest('.process-step-wrap')) closeProcessPopups();
});
